...
def normalize_tensor(tensor):
    ...
    return (tensor - min_val) / (max_val - min_val + 1e-8)  # add epsilon to avoid division by zero




from transformers import AutoModelForSequenceClassification, AutoTokenizer
reward_name = deberta_path
...
rank_model = rank_model.to("cuda:1")
...
def get_ranks_deberta(question, responses):
    all_scores = []
    for response in responses:
        inputs = rank_tokenizer(question, response, return_tensors='pt')
        inputs = {k:inputs[k].to("cuda:1") for k in inputs}
        score = rank_model(**inputs).logits[0].cpu().detach()
        all_scores.append(score)
    return rank_scores(all_scores)
...
def generate(model, tokenizer, input_ids, assistant_role_slice, gen_config=None):
    ...
    ranks = get_ranks_deberta(inputs[0],candidates_texts[0])
